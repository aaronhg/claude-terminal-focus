...
Hook scripts (bash, run by Claude Code process)
  |
  +-> .focus-pending   (per-event signal file, deleted after read)
  |     |
  |     v  FileSystemWatcher
...
**File-based signaling**: Hooks can't talk to VSCode extensions directly. All communication is via JSON files. State file uses atomic writes (`tmp` + `mv`). Signal files (`.focus-pending`, `.focus-signal`) are deleted after read to prevent watcher re-fires.

**Hook write order**: Hooks update state (`_upsert-state.sh`) before writing signal files. This ensures the extension sees the correct state when it reacts to the signal.
...
| `vscode-extension/extension.js` | VSCode extension - watches signal files, sends notifications |
...
| `install.sh` | Copies hooks, extension, and menubar app; merges settings.json; installs LaunchAgent; starts menubar app |
...
- **Notification logic lives in the extension**, not hooks. Extension always sends notifications (no smart skip), checks terminal ownership to avoid duplicates across windows.
...
- **`onDidChangeWindowState`** acks active terminal state when VSCode window regains focus.
...
The menubar app is managed via macOS LaunchAgent (`~/Library/LaunchAgents/com.claude-terminal-focus.menubar.plist`). It starts automatically on login (`RunAtLoad`). Logs go to `/tmp/claude-menubar.log`.

`install.sh` copies the extension and menubar app to `~/.vscode/extensions/` and `~/.claude/menubar-app/` respectively, so deleting the repo does not break the installation.

After install/uninstall, reload VSCode: `Cmd+Shift+P` -> `Reload Window`.

## Editing Hooks

Hook scripts receive JSON on stdin from Claude Code. They extract fields with `jq`, walk the process tree for PID, then source `_upsert-state.sh` to update state, and finally write signal files. The state update must happen before the signal write — the extension reads both and expects consistent state.
